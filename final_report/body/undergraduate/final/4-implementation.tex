...
所有工具都继承自抽象基类 \texttt{BaseTool}，实现规范化的元信息属性 \texttt{name}/\texttt{description}/\texttt{parameters} 与执行方法 \texttt{execute()}。
...
这一抽象有三个工程价值：
其一，所有工具的接口形态统一，DAGPlanner 在校验阶段无需为每个工具单独写适配器；
其二，\texttt{ToolResult} 的 \texttt{data} 字段在第三章 DAG 占位符 \verb|${sX.field}| 解析中作为统一的依赖数据来源；
其三，\texttt{validate\_params} 把参数类型、范围、枚举三类常见错误前移到工具调用前，避免 LLM 输出非法参数时把错误带入工具内部，便于错误根因的诊断。
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\linewidth}{|c|l|X|l|}
        \hline
        类别 & 工具名 & 功能简述 & 关键输出字段 \\ \hline
        \multirow{4}{*}{数值计算} & \texttt{matrix\_operation} & 矩阵基本运算与行列式 & \texttt{result, determinant, shape} \\ \cline{2-4}
                            & \texttt{numerical\_integration} & 自适应数值积分 & \texttt{integral, absolute\_error} \\ \cline{2-4}
                            & \texttt{curve\_fitting} & 多模型曲线拟合（线性/多项式/指数/幂） & \texttt{params, r\_squared, formula} \\ \cline{2-4}
                            & \texttt{equation\_solver} & 区间内方程求根 & \texttt{root, residual} \\ \hline
        \multirow{4}{*}{统计分析} & \texttt{descriptive\_statistics} & 描述性统计量 & \texttt{mean, median, std, q1, q3} \\ \cline{2-4}
                            & \texttt{hypothesis\_test} & 单/双样本 t 检验等 & \texttt{t\_statistic, p\_value, reject\_null} \\ \cline{2-4}
                            & \texttt{linear\_regression} & 一元线性回归 & \texttt{slope, intercept, r\_squared, p\_value} \\ \cline{2-4}
                            & \texttt{correlation\_analysis} & Pearson/Spearman 相关 & \texttt{correlation, p\_value, strength} \\ \hline
        \multirow{4}{*}{可视化} & \texttt{line\_chart} & 折线图 & \texttt{file\_path, width\_pixels} \\ \cline{2-4}
                            & \texttt{scatter\_chart} & 散点图 & \texttt{file\_path, width\_pixels} \\ \cline{2-4}
                            & \texttt{bar\_chart} & 条形图 & \texttt{file\_path, width\_pixels} \\ \cline{2-4}
                            & \texttt{heatmap} & 热力图 & \texttt{file\_path, width\_pixels} \\ \hline
    \end{tabularx}
\end{table}
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\linewidth}{|l|l|X|}
        \hline
        策略类 & 对应配置 & 关键差异 \\ \hline
        \texttt{ReActAgent} & B1, Ours & 经典 ReAct~\cite{yao2023react} 主循环 + Function Calling \\ \hline
        \texttt{CoTReActAgent} & B2 & ReActAgent + CoT 后缀提示\cite{wei2022cot} \\ \hline
        \texttt{PlanAndSolveAgent} & B3 & 两阶段：先生成自然语言计划，再 ReAct 执行\cite{wang2023plansolve} \\ \hline
        \texttt{ReflexionAgent} & B4 & ReAct + 失败后追加自我反思\cite{shinn2023reflexion} \\ \hline
        \texttt{RATSReActAgent} & Ours+RATS & 在 ReActAgent 前插入 RATS 工具筛选 \\ \hline
        \texttt{DAGAgent} & Ours+DAG, Ours\_full & 主循环替换为 Plan-Validate-Execute-Answer 四阶段（\autoref{alg:dag-planner}） \\ \hline
    \end{tabularx}
\end{table}
...
针对 ReAct 在科学场景下的典型失败模式，本文在 \texttt{src/prompts/system\_prompts.py} 中提炼出 4 条提示工程规则，作为 RATS 与 DAGPlanner 在运行时的必要约束（\autoref{tab:o1-o4}）。
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\linewidth}{|c|l|X|}
        \hline
        编号 & 规则名 & 关键内容 \\ \hline
        O1 & 结果合理性检查 & 拟合 $R^2$ 须在 $[0,1]$ 之间；返回 NaN/Inf 视为异常；参数与输入数据主导量级偏差超过 2 个数量级视为异常；异常时下一轮 \emph{必须} 切换方法或工具 \\ \hline
        O2 & 数值计算规范 & 涉及 3 个及以上样本的统计量必须通过 \texttt{descriptive\_statistics} 计算；线性代数运算必须通过 \texttt{matrix\_operation}；最终答复保留至少 4--6 位有效数字 \\ \hline
        O3 & 开场规划模板 & 3 步以上任务的第一轮 \emph{必须} 在 content 中以 \texttt{Plan:} 模板列出步骤；后续每轮简要引用当前进展；执行中允许显式修正 Plan \\ \hline
        O4 & Few-shot 示例注入 & 在 SYSTEM\_PROMPT 末尾追加一条与本题集无关的完整范例轨迹（异常切换 + 最终答复格式），让模型在零样本前已学到本系统期望的行为风格 \\ \hline
    \end{tabularx}
\end{table}

四条规则在叙事上分工明确：O1 负责防止“看起来合理”的工具输出蒙混过关，O2 负责防止 LLM 在标量心算与工具调用之间偷懒，
O3 负责把规划这件事从可有可无变成强制开场，O4 则用一条与目标域同构的 Few-shot 范例统一行为风格。
四条规则共同构成本文方法相对最简基线 B1 的“可解释优势来源”，并在第五章消融实验中得到量化印证。

\subsubsection{DAGPlanner 专用提示}

\texttt{src/prompts/planner\_prompts.py} 定义了 DAGPlanner 专用的三段提示：
...
\textbf{数值正确性}通过正则抽取最终答复中的所有数字，对每个 \texttt{ExpectedNumeric} 用 $\max(\varepsilon_{\text{abs}}, |y^{*}|\cdot\varepsilon_{\text{rel}})$ 双阈值匹配；
\textbf{文件正确性}通过遍历轨迹寻找指定工具的输出 \texttt{file\_path}，并核验文件存在与最小字节数；
\textbf{工具覆盖性}按组列表的 AND/OR 语义判断每组工具是否被成功调用至少一次。
对无工具的 B0 配置，框架仅以数值正确性作为唯一判据，避免由判分维度差异带来的不公比较。

\subsubsection{实验入口与可复现性}

\texttt{scripts/run\_eval.py} 是评测的命令行入口，支持按配置（\texttt{--configs}）与按用例（\texttt{--cases}）的任意组合运行。
每一次评测会在 \texttt{outputs/eval/<timestamp>/} 下落盘 \texttt{results.json} 与 \texttt{summary.md} 两份产物：
